...
public static partial class InterlockedEx {
  private static class HelperForEnum<TValue> {
    ...
    private static CompareExchangeDelegateType _CreateCompareExchangeImplementation() {
      _InitIfNeeded();
      DynamicMethod dynamicMethod = new(string.Empty, _originalType, [_originalType.MakeByRefType(), _originalType, _originalType], true);
    ...
    private static ExchangeDelegateType _CreateExchangeImplementation() {
      _InitIfNeeded();
      DynamicMethod dynamicMethod = new(string.Empty, _originalType, [_originalType.MakeByRefType(), _originalType], true);
      var ilGenerator = dynamicMethod.GetILGenerator();
      ilGenerator.Emit(OpCodes.Ldarg_0);
      ilGenerator.Emit(OpCodes.Ldarg_1);
      ilGenerator.Emit(
        ...
        typeof(Interlocked).GetMethod(
          ...
          BindingFlags.Static | BindingFlags.Public,
          null,
          [_signedType.MakeByRefType(), _signedType],
          null
    ...
    private static HasFlagDelegateType _CreateHasFlagImplementation() {
      _InitIfNeeded();
      DynamicMethod dynamicMethod = new(string.Empty, typeof(bool), [_originalType.MakeByRefType(), _originalType], true);
      var ilGenerator = dynamicMethod.GetILGenerator();
