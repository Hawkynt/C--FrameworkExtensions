...
namespace System.Security.Cryptography {
  ...
  internal
#endif
  class Tiger : HashAlgorithm, IAdvancedHashAlgorithm {
    ...
    private int _BufferLength {
      get { return this.intermediateBuffer == null ? 0 : this.intermediateBuffer.Length; }
    }
    ...
    private static void _Round(ref ulong a, ref ulong b, ref ulong c, ulong x, int mul) {
      var tmp = (c ^= x);
      a -=
        _S0[_GetByteFromQWord(tmp, 0)]
        ^ _S1[_GetByteFromQWord(tmp, 2)]
        ^ _S2[_GetByteFromQWord(tmp, 4)]
        ^ _S3[_GetByteFromQWord(tmp, 6)]
        ;

      tmp = (
        b += _S3[_GetByteFromQWord(tmp, 1)]
        ^ _S2[_GetByteFromQWord(tmp, 3)]
        ^ _S1[_GetByteFromQWord(tmp, 5)]
        ^ _S0[_GetByteFromQWord(tmp, 7)]
      );

      b = tmp * (ulong)mul;
    }

    private static int _GetByteFromQWord(ulong data, int index) {
      return (int)((data >> (index << 3)) & 0xff);
    }

    /* one complete pass */
    private static void _Do8Rounds(ref ulong a, ref ulong b, ref ulong c, ulong[] x, int mul) {
      _Round(ref a, ref b, ref c, x[0], mul);
      _Round(ref b, ref c, ref a, x[1], mul);
      _Round(ref c, ref a, ref b, x[2], mul);
      _Round(ref a, ref b, ref c, x[3], mul);
      _Round(ref b, ref c, ref a, x[4], mul);
      _Round(ref c, ref a, ref b, x[5], mul);
      _Round(ref a, ref b, ref c, x[6], mul);
      _Round(ref b, ref c, ref a, x[7], mul);
    }

    /* The key mixing schedule */
    private static void _KeySchedule(ulong[] x) {
    ...
    private void _Compress(byte[] buf, int offset = 0) {
      ...
      /* load words */
      for (var i = 0; i < 8; i++)
        x[i] = BitConverter.ToUInt64(buf, offset + 8 * i);

      var a = this.state0;
      var b = this.state1;
      var c = this.state2;

      _Do8Rounds(ref a, ref b, ref c, x, 5);
      _KeySchedule(x);
      _Do8Rounds(ref c, ref a, ref b, x, 7);
      _KeySchedule(x);
      _Do8Rounds(ref b, ref c, ref a, x, 9);
    ...
    protected override void HashCore(byte[] array, int ibStart, int cbSize) {
      ...
      if (bufferLength < 1) {
        newBuffer = new byte[cbSize];
        Buffer.BlockCopy(array, ibStart, newBuffer, 0, cbSize);
      } else {
        newBuffer = new byte[cbSize + bufferLength];
      ...
      for (var i = 0; (newBuffer.Length - i) >= 64; i += 64)
        this._Compress(newBuffer, i);

      var rest = newBuffer.Length % 64;
      if (rest < 1) {
        this.intermediateBuffer = null;
        return;
      }

      this.intermediateBuffer = new byte[rest];
      Buffer.BlockCopy(newBuffer, newBuffer.Length - rest, this.intermediateBuffer, 0, rest);
    }
    ...
    protected override byte[] HashFinal() {
       ...
       * encoding like normal. */
        ...
        /* store length */
        buffer.AddRange(BitConverter.GetBytes((ulong)len));
        this._Compress(buffer.ToArray());
      ...
      Buffer.BlockCopy(BitConverter.GetBytes(this.state0), 0, result, 0, 8);
      Buffer.BlockCopy(BitConverter.GetBytes(this.state1), 0, result, 8, 8);
      Buffer.BlockCopy(BitConverter.GetBytes(this.state2), 0, result, 16, 8);
      return (this.OutputBits == 192 ? result : result.Take(this.OutputBits >> 3).ToArray());
    }
    ...
    public string Name { get { return (string.Format("Tiger({0})", this.OutputBits)); } }
