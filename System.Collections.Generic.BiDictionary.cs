...
namespace System.Collections.Generic {
  ...
  internal class BiDictionary<TFirst, TSecond> :
    ...
    public int Count
      => this._firstToSecond.Count
      ;

    object ICollection.SyncRoot
      => ((ICollection)this._firstToSecond).SyncRoot
      ;

    bool ICollection.IsSynchronized
      => ((ICollection)this._firstToSecond).IsSynchronized
      ;

    bool IDictionary.IsFixedSize
      => ((IDictionary)this._firstToSecond).IsFixedSize
      ;

    public bool IsReadOnly
      => this._firstToSecond.IsReadOnly || this._secondToFirst.IsReadOnly
      ;
    ...
    IDictionaryEnumerator IDictionary.GetEnumerator()
      => ((IDictionary)this._firstToSecond).GetEnumerator()
      ;
    ...
    public bool ContainsKey(TFirst key)
      => this._firstToSecond.ContainsKey(key)
      ;

    public bool ContainsValue(TSecond value)
      => this._secondToFirst.ContainsKey(value)
      ;

    public bool Contains(KeyValuePair<TFirst, TSecond> item)
      => this._firstToSecond.Contains(item)
      ;

    public bool TryGetValue(TFirst key, out TSecond value)
      => this._firstToSecond.TryGetValue(key, out value)
      ;

    public bool TryGetKey(TSecond key, out TFirst value)
          => this._secondToFirst.TryGetValue(key, out value)
          ;
    ...
    public bool Remove(KeyValuePair<TFirst, TSecond> item)
      => this._firstToSecond.Remove(item)
      ;

    public bool Contains(object key)
      => ((IDictionary)this._firstToSecond).Contains(key)
      ;
    ...
    private class ReverseDictionary :
      ...
      public int Count
        => this._owner._secondToFirst.Count
        ;

      object ICollection.SyncRoot
        => ((ICollection)this._owner._secondToFirst).SyncRoot
        ;

      bool ICollection.IsSynchronized
        => ((ICollection)this._owner._secondToFirst).IsSynchronized
        ;

      bool IDictionary.IsFixedSize
        => ((IDictionary)this._owner._secondToFirst).IsFixedSize
        ;

      public bool IsReadOnly
        => this._owner._secondToFirst.IsReadOnly || this._owner._firstToSecond.IsReadOnly
        ;
      ...
      public ICollection<TSecond> Keys
        => this._owner._secondToFirst.Keys
        ;
      ...
      IDictionaryEnumerator IDictionary.GetEnumerator()
        => ((IDictionary)this._owner._secondToFirst).GetEnumerator()
        ;
      ...
      public bool ContainsKey(TSecond key)
        => this._owner._secondToFirst.ContainsKey(key)
        ;

      public bool Contains(KeyValuePair<TSecond, TFirst> item)
        => this._owner._secondToFirst.Contains(item)
        ;

      public bool TryGetValue(TSecond key, out TFirst value)
        => this._owner._secondToFirst.TryGetValue(key, out value)
        ;
      ...
      public bool Remove(KeyValuePair<TSecond, TFirst> item)
        => this._owner._secondToFirst.Remove(item)
        ;

      public bool Contains(object key)
        => ((IDictionary)this._owner._secondToFirst).Contains(key)
        ;
