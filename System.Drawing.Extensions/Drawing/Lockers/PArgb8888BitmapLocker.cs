...
/// <summary>
/// Provides span-based access to 32-bit pre-multiplied ARGB bitmap data.
/// </summary>
/// <remarks>
/// Pre-multiplied alpha means R, G, B values are stored multiplied by alpha.
/// This format is optimal for alpha blending operations.
/// </remarks>
internal sealed class PArgb8888BitmapLocker : TypedBitmapLockerBase<Bgra8888> {
  ...
  public override unsafe Color this[int x, int y] {
    ...
    get {
      ...
      return Color.FromArgb(a, (r * 255) / a, (g * 255) / a, (b * 255) / a);
    }
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    set {
      var pointer = (byte*)this._data.Scan0 + (this.Stride * y + x) * 4;
      ...
      if (a == 0) {
        ...
        pointer[0] = (byte)((value.B * a) / 255);
        pointer[1] = (byte)((value.G * a) / 255);
        pointer[2] = (byte)((value.R * a) / 255);
        pointer[3] = a;
      }
    }
  }

  public PArgb8888BitmapLocker(Bitmap bitmap) : this(bitmap, ImageLockMode.ReadWrite) { }

  public PArgb8888BitmapLocker(Bitmap bitmap, ImageLockMode lockMode)
    : base(bitmap, lockMode, 4, PixelFormat.Format32bppPArgb) { }

  public PArgb8888BitmapLocker(Bitmap bitmap, Rectangle rect, ImageLockMode lockMode, PixelFormat format)
    : base(bitmap, rect, lockMode, 4, PixelFormat.Format32bppPArgb, PixelFormat.Format32bppPArgb) { }

  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  protected override Color PixelToColor(Bgra8888 pixel) {
    var a = pixel.A;
    if (a == 0)
      return Color.FromArgb(0, 0, 0, 0);
    if (a == 255)
      return Color.FromArgb(255, pixel.R, pixel.G, pixel.B);
    return Color.FromArgb(a, (pixel.R * 255) / a, (pixel.G * 255) / a, (pixel.B * 255) / a);
  }

  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  protected override Bgra8888 ColorToPixel(Color color) {
    var a = color.A;
    if (a == 0)
      return new Bgra8888(0, 0, 0, 0);
    if (a == 255)
      return new Bgra8888(color.R, color.G, color.B, 255);
    return new Bgra8888(
      (byte)((color.R * a) / 255),
      (byte)((color.G * a) / 255),
      (byte)((color.B * a) / 255),
  ...
  public override unsafe void DrawHorizontalLineUnchecked(int x, int y, int length, Color color) {
    ...
    // 64-bit tier: 2 pixels per ulong
    var packed64 = packed | ((ulong)packed << 32);
    if (count >= 2) {
      var end8 = ptr + (count & ~7);
      while (ptr < end8) {
