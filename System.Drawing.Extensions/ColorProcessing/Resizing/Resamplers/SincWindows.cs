...
file readonly struct SincWindowKernel<TPixel, TWork, TKey, TDecode, TProject, TEncode>(
  ...
  private float Window(float x) => windowType switch {
    WindowType.Blackman => BlackmanWindow(x),
    WindowType.Hann => HannWindow(x),
    WindowType.Hamming => HammingWindow(x),
    WindowType.Kaiser => KaiserWindow(x, param == 0f ? Kaiser.DefaultBeta : param),
    WindowType.Welch => WelchWindow(x),
    WindowType.Bartlett => BartlettWindow(x),
    WindowType.Nuttal => NuttalWindow(x),
    WindowType.BlackmanNuttal => BlackmanNuttalWindow(x),
    WindowType.BlackmanHarris => BlackmanHarrisWindow(x),
    WindowType.FlatTop => FlatTopWindow(x),
    WindowType.Cosine => CosineWindow(x),
    WindowType.PowerOfCosine => PowerOfCosineWindow(x, param == 0f ? PowerOfCosine.DefaultAlpha : param),
    WindowType.Tukey => TukeyWindow(x, param == 0f ? Tukey.DefaultAlpha : param),
    WindowType.Poisson => PoissonWindow(x, param == 0f ? Poisson.DefaultDecay : param),
    WindowType.BartlettHann => BartlettHannWindow(x),
    WindowType.HanningPoisson => HanningPoissonWindow(x, param == 0f ? HanningPoisson.DefaultAlpha : param),
    WindowType.Bohman => BohmanWindow(x),
    WindowType.Cauchy => CauchyWindow(x, param == 0f ? Cauchy.DefaultAlpha : param),
