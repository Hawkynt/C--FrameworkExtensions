...
public readonly struct Clean : IPixelScaler {
  ...
  public TResult InvokeKernel<TWork, TKey, TPixel, TDistance, TEquality, TLerp, TEncode, TResult>(
    ...
    => this._scale switch {
      0 or 2 => callback.Invoke(new Clean2xKernel<TWork, TKey, TPixel, TEquality, TEncode>(equality)),
...
file readonly struct Clean2xKernel<TWork, TKey, TPixel, TEquality, TEncode>(TEquality equality = default)
  ...
  public unsafe void Scale(in NeighborWindow<TWork, TKey> window, TPixel* dest, int destStride, in TEncode encoder) {
    ...
    // NeighborWindow uses [Row][Column] naming: M1P0 = row -1, col 0 = UP
    var c = window.P0P0;   // center (row 0, col 0)
    var u = window.M1P0;   // up (row -1, col 0)
    var d = window.P1P0;   // down (row +1, col 0)
    var l = window.P0M1;   // left (row 0, col -1)
    var r = window.P0P1;   // right (row 0, col +1)
    var ul = window.M1M1;  // up-left (row -1, col -1)
    var ur = window.M1P1;  // up-right (row -1, col +1)
    var dl = window.P1M1;  // down-left (row +1, col -1)
    var dr = window.P1P1;  // down-right (row +1, col +1)
