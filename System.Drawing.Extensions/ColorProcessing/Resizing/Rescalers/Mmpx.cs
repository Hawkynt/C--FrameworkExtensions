...
file readonly struct Mmpx2xKernel<TWork, TKey, TPixel, TEquality, TLerp, TEncode>(TEquality equality = default, TLerp lerp = default)
  ...
  public unsafe void Scale(in NeighborWindow<TWork, TKey> window, TPixel* dest, int destStride, in TEncode encoder) {
    ...
    var a = window.M1M1; // top-left (row -1, col -1)
    var b = window.M1P0; // top (row -1, col 0)
    var c = window.M1P1; // top-right (row -1, col +1)
    var d = window.P0M1; // left (row 0, col -1)
    var e = window.P0P0; // center (row 0, col 0)
    var f = window.P0P1; // right (row 0, col +1)
    var g = window.P1M1; // bottom-left (row +1, col -1)
    var h = window.P1P0; // bottom (row +1, col 0)
    var i = window.P1P1; // bottom-right (row +1, col +1)
