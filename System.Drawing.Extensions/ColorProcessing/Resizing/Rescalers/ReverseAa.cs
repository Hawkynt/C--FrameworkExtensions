...
file readonly struct ReverseAaKernel<TWork, TKey, TPixel, TEncode>
  ...
  public unsafe void Scale(
    ...
    var b1Enc = encoder.Encode(window.M2P0.Work); // 2 above (row -2, col 0)
    var bEnc = encoder.Encode(window.M1P0.Work);  // 1 above (row -1, col 0)
    var dEnc = encoder.Encode(window.P0M1.Work);  // 1 left (row 0, col -1)
    var eEnc = encoder.Encode(window.P0P0.Work);  // center (row 0, col 0)
    var fEnc = encoder.Encode(window.P0P1.Work);  // 1 right (row 0, col +1)
    var hEnc = encoder.Encode(window.P1P0.Work);  // 1 below (row +1, col 0)
    var h5Enc = encoder.Encode(window.P2P0.Work); // 2 below (row +2, col 0)
    var d0Enc = encoder.Encode(window.P0M2.Work); // 2 left (row 0, col -2)
    var f4Enc = encoder.Encode(window.P0P2.Work); // 2 right (row 0, col +2)
