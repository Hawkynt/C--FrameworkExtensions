...
public readonly struct Xbrz : IPixelScaler {
  ...
  public TResult InvokeKernel<TWork, TKey, TPixel, TDistance, TEquality, TLerp, TEncode, TResult>(
    ...
    => this._scale switch {
      2 => callback.Invoke(new Xbrz2xKernel<TWork, TKey, TPixel, TEquality, TDistance, TLerp, TEncode>(equality, default, lerp)),
...
file static class XbrzHelpers {
  ...
  public const float EqualColorTolerance = 30.0f;
  public const float EqualColorToleranceSquared = EqualColorTolerance * EqualColorTolerance;
  ...
  /// </remarks>
  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  public static void PreProcessCorners<TKey, TMetric>(
    in TKey b, in TKey c,
    ...
    var distFG = metric.Distance(f, g);
    var distJK = metric.Distance(j, k);
    var distFJ = metric.Distance(f, j);
    var distGK = metric.Distance(g, k);

    if (distFG < EqualColorToleranceSquared && distJK < EqualColorToleranceSquared)
      return;
    if (distFJ < EqualColorToleranceSquared && distGK < EqualColorToleranceSquared)
      return;
    ...
    if (jg < fk) {
      ...
      if (distFG >= EqualColorToleranceSquared && distFJ >= EqualColorToleranceSquared)
        blendResult.F = dominantGradient ? BlendType.Dominant : BlendType.Normal;
      if (distJK >= EqualColorToleranceSquared && distGK >= EqualColorToleranceSquared)
        blendResult.K = dominantGradient ? BlendType.Dominant : BlendType.Normal;
    } else if (fk < jg) {
      var dominantGradient = DominantDirectionThreshold * fk < jg;
      if (distFJ >= EqualColorToleranceSquared && distJK >= EqualColorToleranceSquared)
        blendResult.J = dominantGradient ? BlendType.Dominant : BlendType.Normal;
      if (distFG >= EqualColorToleranceSquared && distGK >= EqualColorToleranceSquared)
  ...
  /// </summary>
  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  public static byte ComputeBlendInfo<TWork, TKey, TMetric>(
...
file readonly struct Xbrz2xKernel<TWork, TKey, TPixel, TEquality, TMetric, TLerp, TEncode>(TEquality equality = default, TMetric metric = default, TLerp lerp = default)
  ...
  public unsafe void Scale(in NeighborWindow<TWork, TKey> window, TPixel* dest, int destStride, in TEncode encoder) {
    ...
    var blendXy = XbrzHelpers.ComputeBlendInfo(window, metric);
    if (blendXy == 0)
      return;
