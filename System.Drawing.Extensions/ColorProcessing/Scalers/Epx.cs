...
public readonly struct Epx : IPixelScaler, IScalerDispatch {
  ...
  Bitmap IScalerDispatch.Apply(Bitmap source, ScalerQuality quality)
    => quality switch {
      ScalerQuality.Fast => BitmapScalerExtensions.Upscale<
        ...
        EpxKernel<Bgra8888, Bgra8888, Bgra8888, ExactEquality<Bgra8888>, Color4BLerp<Bgra8888>, IdentityEncode<Bgra8888>>
      >(source, new()),
      ScalerQuality.HighQuality => BitmapScalerExtensions.Upscale<
        LinearRgbaF, OklabF,
        Srgb32ToLinearRgbaF, LinearRgbaFToOklabF, LinearRgbaFToSrgb32,
        EpxKernel<LinearRgbaF, OklabF, Bgra8888, ThresholdEquality<OklabF, Euclidean3<OklabF>>, LinearRgbaFLerp, LinearRgbaFToSrgb32>
      >(source, new(new(0.02f))),
      _ => throw new System.NotSupportedException($"Quality {quality} is not supported for EPX.")
    };
  ...
  /// </remarks>
  private readonly struct EpxKernel<TWork, TKey, TPixel, TEquality, TLerp, TEncode>(TEquality equality = default, TLerp lerp = default)
    ...
    public unsafe void Scale(
      ...
      var a = window.M1P0; // top
      var b = window.P0P1; // right
      var c = window.P0M1; // left
      var d = window.P1P0; // bottom
