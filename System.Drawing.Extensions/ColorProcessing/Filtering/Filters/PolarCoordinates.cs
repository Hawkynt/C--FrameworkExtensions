...
/// <summary>
/// Polar coordinates distortion — converts between rectangular and polar coordinate systems.
/// </summary>
...
public readonly struct PolarCoordinates(bool toPolar) : IPixelFilter, IFrameFilter {
  ...
  public TResult InvokeKernel<TWork, TKey, TPixel, TDistance, TEquality, TLerp, TEncode, TResult>(
    ...
    => throw new NotSupportedException("PolarCoordinates requires IFrameFilter dispatch (UsesFrameAccess=true); IPixelFilter direct invocation is not supported. Use Bitmap.ApplyFilter(...) which routes IFrameFilter filters through the resampler pipeline.");
...
file readonly struct PolarCoordinatesFrameKernel<TPixel, TWork, TKey, TDecode, TProject, TEncode>(
  ...
  public unsafe void Resample(
    ...
    if (toPolar) {
      ...
      sx = (int)Math.Floor(cx + r * (float)Math.Cos(theta));
      sy = (int)Math.Floor(cy + r * (float)Math.Sin(theta));
    } else {
      ...
      sx = (int)Math.Floor(theta / (2f * (float)Math.PI) * sourceWidth);
      sy = (int)Math.Floor(r / maxRadius * sourceHeight);
    }
