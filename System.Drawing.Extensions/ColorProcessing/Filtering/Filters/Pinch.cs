...
public readonly struct Pinch(float amount, float radius = 0.5f) : IPixelFilter, IFrameFilter {
  ...
  public TResult InvokeKernel<TWork, TKey, TPixel, TDistance, TEquality, TLerp, TEncode, TResult>(
    ...
    => throw new NotSupportedException("Pinch requires IFrameFilter dispatch (UsesFrameAccess=true); IPixelFilter direct invocation is not supported. Use Bitmap.ApplyFilter(...) which routes IFrameFilter filters through the resampler pipeline.");
...
file readonly struct PinchFrameKernel<TPixel, TWork, TKey, TDecode, TProject, TEncode>(
  ...
  public unsafe void Resample(
    ...
    if (dist < maxDist && dist > 0f) {
      ...
      sx = (int)Math.Floor(cx + dx * scale);
      sy = (int)Math.Floor(cy + dy * scale);
    } else {
      sx = destX;
      sy = destY;
    }
