...
/// <summary>
/// Posterize with edge darkening effect.
/// Quantizes each channel to a configurable number of levels and then darkens
/// pixels proportional to Sobel edge magnitude.
/// Always uses frame-level random access.
/// </summary>
[FilterInfo("PosterEdges",
...
public readonly struct PosterEdges(int levels, float edgeStrength = 0.5f) : IPixelFilter, IFrameFilter {
  ...
  public TResult InvokeKernel<TWork, TKey, TPixel, TDistance, TEquality, TLerp, TEncode, TResult>(
    ...
    => throw new NotSupportedException("PosterEdges requires IFrameFilter dispatch (UsesFrameAccess=true); IPixelFilter direct invocation is not supported. Use Bitmap.ApplyFilter(...) which routes IFrameFilter filters through the resampler pipeline.");
  ...
  public TResult InvokeFrameKernel<TWork, TKey, TPixel, TDecode, TProject, TEncode, TResult>(
    ...
    where TEncode : struct, IEncode<TWork, TPixel>
    => callback.Invoke(new PosterEdgesFrameKernel<TPixel, TWork, TKey, TDecode, TProject, TEncode>(
      this._levels, this._edgeStrength, sourceWidth, sourceHeight));

  public static PosterEdges Default => new();
}

file readonly struct PosterEdgesFrameKernel<TPixel, TWork, TKey, TDecode, TProject, TEncode>(
  int levels, float edgeStrength, int sourceWidth, int sourceHeight)
  ...
  public unsafe void Resample(
    ...
    // Darken by edge magnitude
    var scale = 1f - edgeStrength * edgeMag;
    var or = Math.Max(0f, qr * scale);
    var og = Math.Max(0f, qg * scale);
    var ob = Math.Max(0f, qb * scale);
