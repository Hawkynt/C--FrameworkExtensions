...
/// Removes small bright spots while preserving overall shape.
/// Always uses frame-level random access due to the composed operation requiring double radius.
/// </summary>
[FilterInfo("Opening",
  Description = "Morphological opening (erosion then dilation)", Category = FilterCategory.Morphology)]
public readonly struct Opening(int radius = 1) : IPixelFilter, IFrameFilter {
  ...
  public TResult InvokeKernel<TWork, TKey, TPixel, TDistance, TEquality, TLerp, TEncode, TResult>(
    ...
    => throw new NotSupportedException("Opening requires IFrameFilter dispatch (UsesFrameAccess=true); IPixelFilter direct invocation is not supported. Use Bitmap.ApplyFilter(...) which routes IFrameFilter filters through the resampler pipeline.");
...
file readonly struct OpeningFrameKernel<TPixel, TWork, TKey, TDecode, TProject, TEncode>(
