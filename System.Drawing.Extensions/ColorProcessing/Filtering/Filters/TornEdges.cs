...
public readonly struct TornEdges(float imageBalance = 0.5f, float smoothness = 11f, float contrast = 17f, int seed = 0) : IPixelFilter, IFrameFilter {
  ...
  public TResult InvokeKernel<TWork, TKey, TPixel, TDistance, TEquality, TLerp, TEncode, TResult>(
    ...
    => throw new NotSupportedException("TornEdges requires IFrameFilter dispatch (UsesFrameAccess=true); IPixelFilter direct invocation is not supported. Use Bitmap.ApplyFilter(...) which routes IFrameFilter filters through the resampler pipeline.");
