...
/// <summary>
/// Procedural value noise clouds blended with source image.
/// Uses multi-octave value noise (4 octaves) with configurable scale and blend factor.
/// </summary>
[FilterInfo("Clouds",
...
public readonly struct Clouds(float scale = 0.05f, float blend = 0.5f, int seed = 0) : IPixelFilter, IFrameFilter {
  ...
  public TResult InvokeKernel<TWork, TKey, TPixel, TDistance, TEquality, TLerp, TEncode, TResult>(
    ...
    => throw new NotSupportedException("Clouds requires IFrameFilter dispatch (UsesFrameAccess=true); IPixelFilter direct invocation is not supported. Use Bitmap.ApplyFilter(...) which routes IFrameFilter filters through the resampler pipeline.");
  ...
  public TResult InvokeFrameKernel<TWork, TKey, TPixel, TDecode, TProject, TEncode, TResult>(
    ...
    where TEncode : struct, IEncode<TWork, TPixel>
    => callback.Invoke(new CloudsFrameKernel<TPixel, TWork, TKey, TDecode, TProject, TEncode>(
      this._scale, this._blend, seed, sourceWidth, sourceHeight));

  public static Clouds Default => new();
}

file readonly struct CloudsFrameKernel<TPixel, TWork, TKey, TDecode, TProject, TEncode>(
  ...
  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  internal static float _MultiOctaveNoise(float x, float y, float noiseScale, int noiseSeed) {
    var value = 0f;
    var amplitude = 1f;
  ...
  public unsafe void Resample(
    ...
    r = r * (1f - blend) + noise * blend;
    g = g * (1f - blend) + noise * blend;
    b = b * (1f - blend) + noise * blend;
