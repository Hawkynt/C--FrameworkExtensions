...
/// <summary>
/// Bit-exact verification of <see cref="EbcdicEncoding"/> (CP037) against the authoritative Unicode mapping
/// file https://www.unicode.org/Public/MAPPINGS/VENDORS/MICSFT/EBCDIC/CP037.TXT (all 256 code points,
/// transcribed below as independent golden data).
/// </summary>
[TestFixture]
public class EbcdicGoldenTests {

  // CP037 byte -> Unicode code point, index 0x00..0xFF, from unicode.org CP037.TXT.
  private static readonly int[] _cp037 = [
  ...
  [Test]
  public void Decode_MatchesUnicodeGolden_AllBytes() {
    for (var b = 0; b < 256; ++b)
      Assert.AreEqual((char)_cp037[b], EbcdicEncoding.FromEbcdic((byte)b), $"byte 0x{b:X2}");
  }

  [Test]
  public void Encode_IsInverseOfGoldenDecode_AllBytes() {
    // Every byte must round-trip through char and back (CP037 is a bijection over 0x00..0xFF).
    for (var b = 0; b < 256; ++b)
      Assert.AreEqual((byte)b, EbcdicEncoding.ToEbcdic((char)_cp037[b]), $"byte 0x{b:X2}");
  }
}
