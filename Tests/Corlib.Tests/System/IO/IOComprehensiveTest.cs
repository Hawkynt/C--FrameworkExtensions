...
public class IOComprehensiveTest {
  ...
  [SetUp]
  public void Setup() {
    _tempDirectory = Path.Combine(Path.GetTempPath(), $"IOTests_{Guid.NewGuid():N}");
  ...
  private FileInfo CreateTestFile(string content = "test content", string extension = ".txt") {
    var fileName = Path.Combine(_tempDirectory, $"test_{Guid.NewGuid():N}{extension}");
  ...
  private DirectoryInfo CreateTestDirectory(string subPath = null) {
    var dirPath = subPath != null 
      ? Path.Combine(_tempDirectory, subPath)
      : Path.Combine(_tempDirectory, $"testdir_{Guid.NewGuid():N}");
  ...
  public void FileInfoExtensions_WithNewExtension_ChangesExtension() {
    var originalFile = CreateTestFile();
  ...
  public void FileInfoExtensions_Touch_NonExistentFile_CreatesFile() {
    var nonExistentFile = new FileInfo(Path.Combine(_tempDirectory, "touched.txt"));
    _tempFiles.Add(nonExistentFile.FullName);
  ...
  public void FileInfoExtensions_TryDelete_ExistingFile_DeletesAndReturnsTrue() {
    ...
    _tempFiles.Remove(file.FullName); // Already deleted
  }
  ...
  public void FileInfoExtensions_WriteAllText_CreatesFileWithContent() {
    var file = new FileInfo(Path.Combine(_tempDirectory, "written.txt"));
    _tempFiles.Add(file.FullName);
  ...
  public void FileInfoExtensions_WriteAllLines_CreatesFileWithLines() {
    var file = new FileInfo(Path.Combine(_tempDirectory, "lines.txt"));
    _tempFiles.Add(file.FullName);
  ...
  public void FileInfoExtensions_AppendAllText_AppendsToExistingFile() {
    var file = CreateTestFile("Initial content");
  ...
  public void FileInfoExtensions_IsTextFile_TextFile_ReturnsTrue() {
    ...
    var file = CreateTestFile(textContent);
  ...
  public void FileInfoExtensions_IsTextFile_BinaryFile_ReturnsFalse() {
    ...
    var file = new FileInfo(Path.Combine(_tempDirectory, "binary.bin"));
    _tempFiles.Add(file.FullName);
  ...
  public void FileInfoExtensions_DetectEncoding_UTF8File_DetectsCorrectly() {
    ...
    var file = new FileInfo(Path.Combine(_tempDirectory, "utf8.txt"));
    _tempFiles.Add(file.FullName);
  ...
  public void FileInfoExtensions_DetectLineBreakMode_UnixLineBreaks_DetectsCorrectly() {
    var content = "Line 1\nLine 2\nLine 3\n";
    var file = new FileInfo(Path.Combine(_tempDirectory, "unix.txt"));
    _tempFiles.Add(file.FullName);
  ...
  public void DirectoryInfoExtensions_NotExists_NonExistentDirectory_ReturnsTrue() {
    var nonExistentDir = new DirectoryInfo(Path.Combine(_tempDirectory, "nonexistent"));
  ...
  public void DirectoryInfoExtensions_TryCreate_NonExistentDirectory_CreatesAndReturnsTrue() {
    var newDir = new DirectoryInfo(Path.Combine(_tempDirectory, "newdir"));
    _tempDirectories.Add(newDir.FullName);
  ...
  public void FileInfoExtensions_CopyTo_FileInfo_CopiesToDestination() {
    var sourceFile = CreateTestFile("Source content");
    var destFile = new FileInfo(Path.Combine(_tempDirectory, "destination.txt"));
    _tempFiles.Add(destFile.FullName);
  ...
  public void FileInfoExtensions_CopyTo_DirectoryInfo_CopiesToDirectory() {
    var sourceFile = CreateTestFile("Directory copy content");
    ...
    _tempFiles.Add(copiedFile.FullName);
  }

  [Test]
  public void FileInfoExtensions_CopyTo_OverwriteExisting_OverwritesSuccessfully() {
    var sourceFile = CreateTestFile("New content");
    var destFile = CreateTestFile("Old content");
  ...
  public void FileInfoExtensions_RenameTo_ChangesFileName() {
    var originalFile = CreateTestFile("Rename content");
    ...
    _tempFiles.Add(renamedFile.FullName);
    _tempFiles.Remove(oldName);
  }
  ...
  public void DirectoryInfoExtensions_GetSize_DeepHierarchy_PerformanceTest() {
    var rootDir = CreateTestDirectory();
  ...
  public async Task FileInfoExtensions_WriteAllTextAsync_WritesCorrectly() {
    var file = new FileInfo(Path.Combine(_tempDirectory, "async.txt"));
    _tempFiles.Add(file.FullName);
  ...
  public async Task FileInfoExtensions_CopyToAsync_CopiesToDestination() {
    var sourceFile = CreateTestFile("Async copy content");
    var destFile = new FileInfo(Path.Combine(_tempDirectory, "async_dest.txt"));
    _tempFiles.Add(destFile.FullName);
  ...
  public void FileInfoExtensions_LineOperations_ZeroLines_HandlesCorrectly() {
    var file = CreateTestFile("Single line");
