...
#if !SUPPORTS_INT32_STATICMETHODS
...
public static partial class Int32Polyfills {
  extension(int) {
    ...
    public static int Abs(int value)
      => value == int.MinValue ? throw new OverflowException("Negating the minimum value of a twos complement number is invalid.") : value < 0 ? -value : value;
    ...
    public static int MaxMagnitude(int x, int y) {
      var absX = x < 0 ? (x == int.MinValue ? int.MaxValue : -x) : x;
      var absY = y < 0 ? (y == int.MinValue ? int.MaxValue : -y) : y;
    ...
    public static int CopySign(int value, int sign) {
      var absValue = value < 0 ? (value == int.MinValue ? throw new OverflowException() : -value) : value;
