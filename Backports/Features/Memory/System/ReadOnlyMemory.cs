...
public readonly struct ReadOnlyMemory<T> : IEquatable<ReadOnlyMemory<T>> {
  ...
  public ReadOnlySpan<T> Span {
    ...
    get {
      ...
      if (this._object is T[] array)
        return new ReadOnlySpan<T>(array, this._index, this._length);

      if (this._object is string str && typeof(T) == typeof(char)) {
        unsafe {
          fixed (char* ptr = str)
            return new ReadOnlySpan<T>((T*)(void*)(ptr + this._index), this._length);
        }
      }
  ...
  public override bool Equals(object obj) =>
    obj is ReadOnlyMemory<T> other && this.Equals(other);
  ...
  public override string ToString() {
    if (typeof(T) == typeof(char)) {
      if (this._object is string str)
        return str.Substring(this._index, this._length);
      if (this._object is T[] array)
        return new string((char[])(object)array, this._index, this._length);
    }

    return $"System.ReadOnlyMemory<{typeof(T).Name}>[{this._length}]";
  }
  ...
  /// <summary>
  /// Defines an implicit conversion of an ArraySegment to a <see cref="ReadOnlyMemory{T}"/>.
  /// </summary>
  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  public static implicit operator ReadOnlyMemory<T>(ArraySegment<T> segment) => new(segment.Array, segment.Offset, segment.Count);
